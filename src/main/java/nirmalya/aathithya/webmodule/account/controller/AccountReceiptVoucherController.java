...
public class AccountReceiptVoucherController {
	...
	public @ResponseBody List<AccountJournalVoucherModel> viewReceiptVoucher(HttpSession session) {
		...
		try {
			resp = restClient.getForObject(env.getAccountUrl() + "/viewReceiptVoucher", JsonResponse.class);
		} catch (RestClientException e) {
			e.printStackTrace();
		}
	...
	// view-account-edit
		...
		public @ResponseBody JsonResponse<List<AccountModel>> editReceiptInfo(Model model, @RequestParam String id,
				HttpSession session) {
			...
			try {
				jsonResponse = restClient.getForObject(env.getAccountUrl() + "/editReceiptInfo?id=" + id,
						JsonResponse.class);
		...
		public @ResponseBody JsonResponse<List<DropDownModel>> vouchernumber(HttpSession session) {
			...
			try {
				resp = restClient.getForObject(env.getAccountUrl() + "/getReceiptvoucherNumber", JsonResponse.class);
			} catch (RestClientException e) {
				e.printStackTrace();
			}
		...
		public @ResponseBody JsonResponse<Object> deleteJournalDetails(@RequestParam String id, HttpSession session) {
			...
			try {
				res = restClient.getForObject(env.getAccountUrl() + "deleteReceiptDetails?id=" + id, JsonResponse.class);
			} catch (RestClientException e) {
				e.printStackTrace();
			}
